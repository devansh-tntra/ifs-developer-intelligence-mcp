...
BEGIN
   FOR rec_ IN top_customers_cur LOOP
      customer_labels_ := customer_labels_ || TO_CHAR(rec_.customer_id) || field_separator_;
      order_amounts_ := order_amounts_ || TO_CHAR(rec_.Order_total) || field_separator_;
   END LOOP;
...
BEGIN
   FOR rec_ IN top_products_cur LOOP
      product_label_ := product_label_ || TO_CHAR(rec_.Product_id) || field_separator_;
      order_quantity_ := order_quantity_ || TO_CHAR(rec_.Order_quantity) || field_separator_;
   END LOOP;
...
BEGIN
   FOR rec_ IN top_branches_cur LOOP
      branch_label_ := branch_label_ || TO_CHAR(rec_.branch_id) || field_separator_;
      order_amounts_ := order_amounts_ || TO_CHAR(rec_.Order_total) || field_separator_;
   END LOOP;
...
BEGIN
   FOR rec_ IN country_order_total_cur LOOP
      country_label_ := country_label_ || TO_CHAR(rec_.Country_id) || field_separator_;
      order_amounts_ := order_amounts_ || TO_CHAR(rec_.Order_total) || field_separator_;
      count_ := count_+1;
   END LOOP;
...
BEGIN
   FOR rec_ IN total_order_monthly_cur LOOP
        time_label_ := time_label_ || TO_CHAR(rec_.Time_period) || field_separator_;
        order_amounts_ := order_amounts_ || TO_CHAR(rec_.Order_total) || field_separator_;
        count_ := count_+1;
   END LOOP;
