...
export async function indexWorkspaceDirectory(targetDir) {
    ...
    try {
        ...
        for (const relPath of files) {
            ...
            let assetType = 'other';
            if (['entity', 'model'].includes(ext))
                assetType = 'entity';
            else if (ext === 'projection')
                assetType = 'projection';
            else if (['client', 'page', 'fragment'].includes(ext))
                assetType = 'client';
            else if (['api', 'apy', 'plsql'].includes(ext))
                assetType = 'plsql';
            else if (ext === 'views')
                assetType = 'views';
            else if (ext === 'storage')
