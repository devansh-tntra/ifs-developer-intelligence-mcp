...
export function parsePlsqlPackage(content: string, filename: string): PlsqlPackage {
  ...
  const procedures: PlsqlPackage['procedures'] = [];
  ...
  const functions: PlsqlPackage['functions'] = [];
...
export function extractErrorSysMessages(content: string): { key: string; text: string }[] {
