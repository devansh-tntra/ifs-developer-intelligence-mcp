...
export function analyzeObjectImpact(objectName: string): ImpactAnalysisResult {
  const nameUpper = objectName.toUpperCase().replace(/\.(entity|projection|client|api|apy)$/, '');
  const assets = db.getAllWorkspaceAssets();

  const downstreamAffectedObjects: DependencyNode[] = [];
  const directUpstreamDependencies: DependencyNode[] = [];
  ...
  for (const asset of matchingAssets) {
    let type: DependencyNode['type'] = 'Package';
    if (asset.extension === 'entity') type = 'Entity';
    else if (asset.extension === 'projection') type = 'Projection';
    else if (asset.extension === 'client' || asset.extension === 'page') type = 'Entity';
  ...
  let riskLevel: ImpactAnalysisResult['riskLevel'] = 'MEDIUM';
  if (downstreamAffectedObjects.length > 5) riskLevel = 'CRITICAL';
  else if (downstreamAffectedObjects.length > 3) riskLevel = 'HIGH';
